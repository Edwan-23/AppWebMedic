...
const Calendar: React.FC = () => {
  ...
  const cargarEventos = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/eventos-calendario?usuario_id=${usuario.id}`);
      ...
      const eventosFormateados = data.eventos.map((evento: any) => ({
        ...
        allDay: true,
        extendedProps: { 
          calendar: calendarsEvents[evento.color as keyof typeof calendarsEvents] || "primary" 
        },
      }));
  ...
  const handleAddOrUpdateEvent = async () => {
    ...
    try {
      ...
      if (selectedEvent) {
        ...
        if (!response.ok) {
          console.error("Error de validación:", data.detalles);
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/eventos-calendario?id=${selectedEvent.id}`, {
        method: "DELETE",
      });
        ...
        <div className="flex flex-col px-2 overflow-y-auto custom-scrollbar">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Programa un evento para recordar nuevas publicaciones, solicitudes o envíos.
            </p>
          </div>
          <div className="mt-8">
            <div>
          ...
          <div className="flex items-center gap-3 mt-6 modal-footer sm:justify-end">
            {selectedEvent && (
              <button
                ...
                type="button"
                disabled={loading}
                className="flex w-full justify-center rounded-lg border border-red-300 bg-white px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 dark:border-red-700 dark:bg-gray-800 dark:text-red-400 dark:hover:bg-red-900/20 sm:w-auto disabled:opacity-50"
              >
                Eliminar
              </button>
            )}
            ...
            >
              {loading ? "Guardando..." : selectedEvent ? "Actualizar Cambios" : "Agregar Evento"}
            </button>
          </div>
        </div>
      </Modal>
